...
for i in "${S[@]}"
do
#	pigz -d $ARG_A/$i.trinity.Trinity.fasta.gz
	makesomethingNotInterleaved.pl $ARG_A/$i.trinity_22_merged100_contigs.fa > $ARG_A/$i.trinity.NI.fasta &&
...
#	pigz -f $ARG_A/$i.trinity.Trinity.fasta
...
#                                                                                                 #
###################################################################################################
if [ "$ARG_M" = "all" ] || [ "$ARG_M" = "subsetancestortrim" ]; then
taxa=$(wc -l < "$ARG_S") 
genes=$(wc -l < "$ARG_G")
subsetgenes=$(wc -l < "$ARG_X")

mkdir -p $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes} &&
mkdir -p $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes} &&
mkdir -p $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes} &&

#perl -i -nle "s/N|~|n/-/g unless $. & 1; print " $PWD/3_consensus.T${taxa}.G${genes}/${genesubset}.T$taxa.G$genes.03.50.15.1.0.fasta &&
#selectSites.pl -x 3 \
#	$PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.fasta \
#	> $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.trim.fasta &&


esweep_linux_v1 \
	-i $PWD/3_consensus.T${taxa}.G${genes} \
	-o $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes} \
	-n $PWD/$ARG_T \
	-k $PWD/$ARG_X \
	-d %s.T$taxa.G$genes.03.50.15.1.0.fasta \
	-x \
...
cat $ARG_X | while read genesubset
do
#	wc -L $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.fasta > $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.len1.txt &&
	# The following will apply the changes in line numbers 2,4,6,8,......
	perl -i -nle "s/N|~|n/-/g unless $. & 1; print " $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.fasta &&
	selectSites.pl -x 3 \
		$PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.fasta \
		> $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.trim.fasta &&
	...
	FastML_Wrapper.pl \
		--MSA_File $PWD/4_SubsetFilterBait.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}_unaligned.trim.fasta \
		--Tree RAxML_bipartitionsBranchLabels.combined_codon.GTRGAMMA.4_SubsetFilterBait.T14.G11770.S500.NOG.nwk \
		--seqType NUC \
		--outDir $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset} &&
	cp $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}/seq.joint.txt $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.joint.fas &&
	cp $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}/seq.marginal.txt $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.fas &&
	rm -rf $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset} &&
	
	cat $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.fas > $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.fas &&
	extractSequence_beginingMatch.pl \
		$PWD/3_consensus.T${taxa}.G${genes}/${genesubset}.T$taxa.G$genes.03.50.15.1.0.fasta \
		Bombyx_mori_ASM15162v1_cds_all >> $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.fas &&

	perl -i -nle "s/-|~|n/N/g unless $. & 1; print " $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.fas &&
	degapcodon2.pl $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.fas \
		> $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.DG.fas &&

	alignAM.py \
		-prot_outfile $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_aa.fasta \
		-nuc_outfile $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_nt.fasta \
		-aligner mafft -options " --localpair --maxiterate 1000 --reorder --anysymbol --thread -1 " \
		$PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.DG.fas &&

	perl -i -nle "s/N|~|n/-/g unless $. & 1; print " $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_nt.fasta &&
	selectSites.pl -x 3 $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_nt.fasta \
		> $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_nt.trim.fasta
	if [ $count -eq 0 ] 
	then 
		grep '^>' $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes}/${genesubset}.seq.marginal.plusog.mafft_nt.trim.fasta | awk 'sub(/^>/, "")' | awk -F'~' '{print $1}' > taxa.OG.Ancestor.txt
	fi

	echo "Bombyx_mori_ASM15162v1_cds_all" >> taxa.OG.Ancestor.txt &&
 
	let count=count+1;
done
head -n 1 $ARG_F > ${subsetgenes}.$ARG_F && 
awk -F '\t' 'NR==FNR {id[$1]; next} $1 in id' $ARG_G $ARG_F >> ${subsetgenes}.$ARG_F

ulimit -n 25000 &&
esweep_linux_v2 \
	-i $PWD/5_ancestral.T${taxa}.G${genes}.S${subsetgenes} \
	-o $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes} \
	-s $PWD/taxa.OG.Ancestor.txt \
...
do
	paste -sd'\t\n' $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_unaligned.fasta  | tr -d '>' | tr -d '\r' > $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_unaligned.txt
	awk '{t[NR]=$1;s[NR]=$2} END{r=1; while(r<15){printf ">%s\n",t[r]; for(i=1;i<=length(s[1]);i++) {if(substr(s[r],i,1)~/[ACGT-]/) printf substr(s[r],i,1); else printf substr(s[8],i,1)} printf "\n";r++}}' $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_unaligned.txt > $PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_substit.fasta
done

	if [ "$ARG_B" = "max" ]; then
		mkdir -p $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX &&
		tail -n +2 $PWD/${subsetgenes}.$ARG_F  |sort -k1,1 -k3,3gr | sort -u -k1,1 --merge |cut -f 2 > $PWD/${subsetgenes}.$ARG_F.biggestexonname &&

		cat T${taxa}.plusancestral.txt | while read TAXA
		do
			> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.fasta
			cat $PWD/${subsetgenes}.$ARG_F.biggestexonname | while read EXON
			do
				filterbyname.sh \
					in=$PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_substit.fasta \
					out=stdout.fasta \
					include=t \
					names=${TAXA} >> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.fasta &&
				sed -i "s/>$TAXA/>${EXON}_${TAXA}/g" $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.fasta
			done
			makesomethingNotInterleaved.pl $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.fasta \
				> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.NI.fasta &&
			perl -i -nle "s/N//g unless $. & 1; print " $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.NI.fasta &&
			reformat.sh \
				in=$PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.NI.fasta \
				out=$PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.MAX/${TAXA}.preBAIT.MAX.MIN100.fasta \
				minlen=100 \
				ow=t \
				fastawrap=1000000
		done
		...
		mkdir -p $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test &&
#		tail -n +2 $PWD/${subsetgenes}.$ARG_F | cut -f 2 > $PWD/${subsetgenes}.$ARG_F.exonnames &&
		cat T${taxa}.plusancestral.txt | while read TAXA
#		cut -f 2 $PWD/${subsetgenes}.$ARG_F >$PWD/${subsetgenes}.$ARG_F.exonnames
		do 
			> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.fasta
			cat $PWD/${subsetgenes}.$ARG_F.exonnames | while read EXON
			do
				selectSeqs.pl -m ${TAXA} \
					$PWD/5_ancestral.T${taxa}.E${genes}.S${subsetgenes}/${EXON}_substit.fasta \
					>> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.fasta &&
				sed -i "s/>$TAXA/>${EXON}_${TAXA}/g" $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.fasta
			done
			makesomethingNotInterleaved.pl $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.fasta \
				> $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.NI.fasta &&
			perl -i -nle "s/N|-//g unless $. & 1; print " $PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.NI.fasta &&
			reformat.sh \
				in=$PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.NI.fasta \
				out=$PWD/6_preBait.T${taxa}.E${genes}.S${subsetgenes}.test/${TAXA}.preBAIT.MIN100.fasta \
				minlen=100 \
				ow=t \
				fastawrap=1000000
		done
